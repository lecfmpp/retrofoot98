...
/* MEDIDO no manequim, e' o que dita o tamanho: a faixa da GOLA ocupa so' os
   primeiros ~2% da altura dele (~1,75% do quadro) — depois ja' e' o corpo da
   camisa. Os moldes pintam essa faixa com a cor secundaria do clube (o estilo
   `mangas` tem gola e punhos na segunda cor), entao um decote fundo apagaria
   exatamente a parte customizavel.

   O decote fica RASO E LARGO: arredonda a borda reta do topo e deixa a gola
...
/* A CURVA DO TOPO FOI REMOVIDA. Ela guardava o que estava DENTRO de uma
   elipse para arredondar as pontas de cima — mas elipse e' fechada: guardar o
   dentro arredonda o topo E corta o fundo junto. Nao da' para ter um sem o
   outro, e o corte embaixo estragava mais do que a quina de cima incomodava.
   Sobra o decote, que e' concavo e so' mexe na borda de cima. */
...
/* A GOLA VEM PRONTA DO UNIFORME. Tentei desenha-la como camada propria — um
   ...
   nao como gola. O molde ja' traz a faixa dela pintada; o decote raso so'
   arredonda a borda e deixa a faixa aparecendo em volta. */
...
async function montarFotoJogador(rostoUrl, manequimUrl, medida, ajuste){
  ...
  /* `larg` E' RAIO, nao diametro — e' assim que radial-gradient a interpreta no
     CSS, e foi no CSS que as medidas foram calibradas. Eu dividia por 2 aqui,
     entao a elipse do canvas saia com metade da largura e recortava a camisa
     num oval: a previa ficava certa e a imagem salva, errada. */
  const recorte = (larg, fundo, suave) => {
    if(!(larg > 0 && fundo > 0)) return;
    const rx = larg*FOTO_W, ry = fundo*FOTO_H;
...
/* "REFAZER MOLDES" E "MONTAR CAMISAS" SAIRAM, com as funcoes e as constantes
   delas. O primeiro regerava os 5 moldes como camisa sozinha, para encaixar
   cabeca — abordagem abandonada, e ele MEXERIA na camisa do campo, que hoje
   esta' certa. O segundo assava manequim + escudo numa imagem por clube
   (`camisaPronta`), para a montagem por transferencia; conferido: nenhuma
   linha do jogo lia esse campo.
   O 🛡 de posicionar o escudo no manequim saiu junto — ele so' servia ao
   segundo. Ficam "Preparar estilos" e "Repintar todos": a camisa do campo, na
   Formacao, sai dos moldes por repintura local. */
