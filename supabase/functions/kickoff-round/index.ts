...
/* ==================================================================
   kickoff-round — pré-resolvedor da rodada (FASE 2 da Resenha determinística).
   Chamado por QUALQUER cliente no apito (idempotente): simula UMA vez, no
   servidor, TODAS as partidas de liga da rodada (4 divisões) a partir do
   shared_state + snapshot do apito (games.kickoff_lineups), e grava os streams
   de eventos em round_events. Os clientes REPRODUZEM esses streams na tela ao
   vivo (em vez de simular cada um por conta própria) e o resolve-round consome
   os MESMOS resultados ao fechar a rodada — o que foi assistido é o que fica
   gravado, por construção. Partida cujo dono humano joga ao vivo continua
   autoritativa pelo resultado publicado (o stream dela aqui é só o fallback de
   ausente, idêntico ao que o resolve-round usaria).
   Motor de partida colado de resolve-round/index.ts (fonte única — o bloco
   MATCH_ENGINE abaixo é extraído byte a byte de lá pelo script de montagem).
   ================================================================== */
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const gameId = body?.gameId; const round = body?.round;
    if (!gameId || round == null) return json({ error: "gameId/round ausentes" }, 400);
    ...
    const { data: game } = await admin.from("games").select("host_id, shared_state, kickoff_lineups").eq("id", gameId).maybeSingle();
    if (!game) return json({ error: "sala não encontrada" }, 404);
    if (!seat && game.host_id !== user.id) return json({ error: "não é membro da sala" }, 403);

    const stateObj = game.shared_state;
    // sem estado salvo ainda (começo de sala): nada a pré-computar — o cliente simula localmente
    // como sempre (fallback), sem erro. Idem se a rodada pedida não é a do estado (cliente atrasado).
    if (!stateObj || !stateObj.S) return json({ ok: false, nostate: true });
    const S = stateObj.S;
    if (S.round !== round) return json({ ok: false, stale: true, round: S.round });

    // idempotência: primeiro a chegar computa; os demais recebem o MESMO payload gravado
    const { data: existing } = await admin.from("round_events").select("payload").eq("game_id", gameId).eq("round", round).maybeSingle();
    if (existing && existing.payload) return json({ ok: true, round, matches: existing.payload.matches || {} });

    // humanos da sala: assento + snapshot do APITO por cima (mesma precedência do resolve-round —
    // a foto oficial da rodada vence o last_xi/last_tactic atual do assento)
    const { data: seats } = await admin.from("game_seats").select("user_id, club_id, last_xi, last_tactic").eq("game_id", gameId);
    const humanClubs = new Set<string>(); const humanXI: any = {}; const humanTactic: any = {};
    (seats || []).forEach((s: any) => {
    ...
    const kickSnap = (game as any).kickoff_lineups || null;
    ...
    // simula TODAS as partidas de liga da rodada — divisão âncora + outras 3 — com as MESMAS
    // seeds que o cliente/resolve-round usam. Inclui as partidas de clubes humanos: esse stream
    // é o "fallback de ausente" (e o que os espectadores assistem); o resultado publicado pelo
    // dono, quando existir, vence tanto no cliente (pub) quanto no resolve-round (humanResultByFx).
    const seed = S.seed, season = S.season;
    const matches: any = {};
    const sim = (h: string, a: string, mseed: number, div: string) => {
      const r = ME.simMatchPure(h, a, sideInputs(S, h, humanClubs.has(h), humanXI, humanTactic), sideInputs(S, a, humanClubs.has(a), humanXI, humanTactic), mseed, {});
      matches[h + "-" + a] = { hg: r.hg, ag: r.ag, scorers: r.scorers || [], events: r.events || [], perf: r.perf || null, div };
    };
    ((Array.isArray(S.sched) && S.sched[round]) || []).forEach((fx: any) => {
      const h = fx[0], a = fx[1]; if (h == null || a == null) return;
      sim(h, a, ME.hashSeed(seed, round, h, a), S.division);
    });
    for (const d in (S.otherDivs || {})) {
      const od = S.otherDivs[d]; if (!od || !od.sched || !od.sched.length) continue;
      const base = hashC("rnd" + season + "-" + round + "-" + d);
      (od.sched[round % od.sched.length] || []).forEach((fx: any) => {
        const h = fx[0], a = fx[1]; if (h == null || a == null || !od.table[h] || !od.table[a]) return;
        sim(h, a, (base + hashC(h) + hashC(a)) >>> 0, d);
      });
    }

    const payload = { matches };
    const { error: insErr } = await admin.from("round_events").insert({ game_id: gameId, round, payload });
    if (insErr) {
      // corrida: outro cliente inseriu primeiro — devolve o payload DELE (fonte única; o conteúdo
      // é determinístico e idêntico, mas a linha gravada é a autoridade)
      const { data: won } = await admin.from("round_events").select("payload").eq("game_id", gameId).eq("round", round).maybeSingle();
      if (won && won.payload) return json({ ok: true, round, matches: won.payload.matches || {} });
    }
    return json({ ok: true, round, matches });
